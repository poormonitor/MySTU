...
const toolbarConfig = {
    excludeKeys: ["codeBlock", "insertLink", "insertTable", "insertVideo", "group-image", "todo", "blockquote", "fullScreen"]
}
            ...
            <div id="memo" class="col-span-5 md:col-span-2 lg:col-span-1 infoTab overflow-x-hidden">
                <div class="p-4 flex items-center gap-x-2">
                    ...
                    <t-switch v-model="memoEditing" size="large">
                        <template #label="slotProps">{{ slotProps.value ? '修改' : '查看' }}</template>
                    </t-switch>
                </div>
                <div id="memoEdit" v-if="memoEditing">
                    <Toolbar :editor="editorRef" :defaultConfig="toolbarConfig" mode="simple" />
                    <div class="h-[30vh]">
                        <Editor class="overflow-hidden border-b" v-model="memoContent" :defaultConfig="editorConfig"
                            @onCreated="handleCreated" />
                    </div>
                    <div class="flex justify-center mt-6">
                        <t-button @click="submitMemo" :loading="loadingSubmit" size="large">
                            提交
                        </t-button>
                    </div>
                </div>
