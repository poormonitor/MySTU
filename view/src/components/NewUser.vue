...
const rules = {
    user: [{ required: true, message: '姓名必填', type: 'error' },
    {
        min: 2,
        message: '至少需要两个字',
        type: 'error',
    ...
    id: [{ required: true, message: '用户名必填', type: 'error' },
    {
        min: 3,
        message: '至少需要两个字符',
        type: 'error',
                ...
                <t-form-item label="用户名" name="id">
                    <t-input placeholder="用户名" v-model="newUser.id" type="text" autocomplete="username">
                        <template #prefix-icon>
                            <icon-font name="user-avatar" />
                        </template>
                    </t-input>
                </t-form-item>
                <t-form-item label="姓名" name="user">
                    <t-input placeholder="姓名" v-model="newUser.user" type="text" autocomplete="nick">
                        <template #prefix-icon>
                            <icon-font name="user-circle" />
                        </template>
                    </t-input>
                </t-form-item>
                <t-form-item label="密码" name="passwd">
                    <t-input placeholder="密码" v-model="newUser.passwd" type="password" autocomplete="new-password">
                        <template #prefix-icon>
                            <icon-font name="lock-on" />
                        </template>
                    </t-input>
                </t-form-item>
                <t-form-item label="管理员" name="admin">
                    <t-switch v-model="newUser.admin" size="large" :label="['是', '否']"></t-switch>
                </t-form-item>
            </div>
        </t-form>
    </t-dialog>
